...
export async function POST(req:NextRequest){
 try{
  ...
  for(let f=0;f<folds;f++){const testStart=220+f*foldSize,testEnd=f===folds-1?n:testStart+foldSize;const trainEnd=Math.max(220,testStart-embargo);
  ...
  return NextResponse.json({protocol:"chronological walk-forward with embargo",folds:out,averageAccuracy:out.length?out.reduce((s,x)=>s+(x.accuracy||0),0)/out.length:0,note:"This is a validation harness, not proof of future profitability. The final holdout must remain untouched."});
