 ...
 return <div className="app">
  ...
  <main>
   ...
   <section className="grid">
    ...
    <div className="center"><div className="chartCard"><div className="chartHead"><div><small>PRICE ACTION</small><b>{symbol} · {interval}</b></div><span>{streaming?"● WEBSOCKET STREAM":"● PROVIDER DATA"}</span></div><div className="chart">{data.length?<Chart data={data}/>:<div className="empty">{loading?"Connecting to verified market…":"Search a market to load its live chart."}</div>}</div><div className="foot"><span>◉ VERIFIED LIVE DATA</span><span>{data.length} candles</span><span>{provider||"—"}</span></div></div>
